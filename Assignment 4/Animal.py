...
class Animal:
    def __init__(self, terrain, id, position=()):
        ...
        if len(position) == 0: # random generate location
    ...
    def starve(self): # Q1
        ...
        # update hunger lvl by 1 if animal does not eat
    ...
    def grow(self): # Q3
        ...
        # WRITE YOUR CODE HERE FOR QUESTION 3 (2 lines of code)
        
        # age animal if not at max age
    ...
    def die(self): # Q4
        ...
        # WRITE YOUR CODE HERE FOR QUESTION 4 (1-2 lines of code)
        
        # animal dies if too old or starved
        if self.age == self.age_max or self.hunger == self.hunger_max:
            return True
        
    def get_neighbor_positions(self, terrain): # Q5
        """
        Args:
            self: the animal object
        ...
        # WRITE YOUR CODE HERE FOR QUESTION 5 (20-30 lines of code)
        
        horizontal_move = [-1,0,1] # left, stay, right
        vertical_move = [-1,0,1] # down, stay, up
        
        # iterate through all horizonal moves
        for i in horizontal_move:
            new_x = self.position.x + i
            # checks if within terrain domain
            if new_x >= 0 and new_x < terrain.width:
                # iterate through all vertical moves
                for j in vertical_move:
                    new_y = self.position.y + j
                    # checks if within terrain range
                    if new_y >= 0 and new_y < terrain.height:
                        
                        # get position objects using Position Class and corresponding coordinates
                        p1 = Position(0, terrain.width, 0, terrain.height, 
                        new_x, new_y, random_init=False)
                        coordinates = terrain.map[new_x][new_y]
                        # booleans to check occupancy of test coordinates
                        search_pray = re.search(r'(pray.*)', coordinates)
                        search_pred = re.search(r'(pred.*)', coordinates)
                        search_plant = re.search(r'(plant.*)', coordinates)
                        
                        # position objects only appended if coordinates are vacant
                        if terrain.map[new_x][new_y] == "": # empty space
                            avail_neighbor_positions.append(p1)
                        if search_pray: # occupied by prey
                            continue
                        if search_pred: # occupied by predator
                            continue
                        if search_plant: # occupied by plant
                            avail_neighbor_positions.append(p1)
        
        return avail_neighbor_positions
    
    
    def will_spawn(self, terrain): # Q6
        """
        Args:
            self: the animal object
            terrain: the object containing all information about the simulation
        ...
        # WRITE YOUR CODE HERE FOR QUESTION 6 (10-20 lines of code)
        
        if self.age >= self.age_spawn_min and self.age <= self.age_spawn_max: # conditional (1)
            if self.hunger <= 2: # conditional (2)
                if self.spawn_waiting == 0: # conditional (3)
                    if len(self.get_neighbor_positions(terrain)) > 0: # conditional (4)
