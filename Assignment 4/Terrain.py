...
class Terrain:
    ...
    def update_prays(self): # Q9
        ...
        # WRITE YOUR CODE HERE FOR QUESTION 9 (20-30 lines of code)
        
        prayID_list = [prayID for prayID in self.prays.keys()]

        for prayID in prayID_list:
            neighbor_list = self.prays[prayID].inspect(self) # inspects to get list of neighbours within visual range
            prev_pos,curr_pos = self.prays[prayID].move(self, neighbor_list) # moves to get a tuple containing prev and curr pos
            self.update_terrain(prev_pos, curr_pos, prayID) # update map according to prev and curr pos                                 

            # checks whether it will spawn new offspring
            if self.prays[prayID].will_spawn(self): # will_spawn is True             
                spawn_location = self.prays[prayID].get_spawn_location(self) # spawn location
                prays = Pray(terrain=self, id="pray"+str(self.nb_prays_ever_lived), position=spawn_location)
                self.add_pray(prays) # new pray added to dict
                print(f"{prays.id} was born")
                    
            self.prays[prayID].grow()

        prayID_list = [prayID for prayID in self.prays.keys()]
        
        # checks if pray will die
        for prayID in prayID_list:
    ...
    def update_stats(self): # Bonus question
        """
            Args:
        ...
        # WRITE YOUR CODE HERE FOR THE BONUS QUESTION (6 lines of code)
        ...
        self.nb_avail_plants_over_time.append(len(available_plants))
